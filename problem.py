...
def preprocess():
    ...
    interventions_features = pd.read_csv('data/interventions-hebdo-2010-2017.csv', sep=';')
    interventions_features_test = pd.read_csv('data/interventions-sdis91.csv', encoding='ISO-8859-1', sep=';')  
    ...
    interventions_features_test = mise_en_forme_date(interventions_features_test)

    # Remplacer les valeurs NAN par 'AUTR' dans interventions_features_test
    interventions_features_test['ope_categorie'] = interventions_features_test['ope_categorie'].fillna('AUTR')
    ...
    for code_insee in interventions_features_test['ope_code_insee'].unique():
        nom_commune = interventions_features_test[interventions_features_test['ope_code_insee'] == code_insee]['ope_nom_commune'].unique()[0]
        communes_list.append({'code_insee': code_insee, 'nom_commune': nom_commune})

    communes = pd.concat([communes, pd.DataFrame(communes_list)], ignore_index=True)
    ...
    for date in interventions_features['date'].unique():
        for code_insee in interventions_features_test['ope_code_insee'].unique():
            new_row = pd.DataFrame({'semaine': [date], 'code_insee': [code_insee]})
    ...
    def implement_target(df, y):
        ...
        for date in df['date'].unique():
            for code_insee in df['ope_code_insee'].unique():
                for t in type_ope:
                    df_ = df[(df['date'] == date) & (df['ope_code_insee'] == code_insee)]

                    if not df_.empty:
                        matching_rows = df_[df_['ope_categorie'] == t]
                        if not matching_rows.empty:
                            y.loc[(y['semaine'] == date) & (y['code_insee'] == code_insee), 'nb_ope_' + t] = matching_rows['nb_ope'].values[0]
                        else:
                            y.loc[(y['semaine'] == date) & (y['code_insee'] == code_insee), 'nb_ope_' + t] = 0
                    else:
                        y.loc[(y['semaine'] == date) & (y['code_insee'] == code_insee), 'nb_ope_' + t] = 0
        return y
    ...
    if not communes_features['code_insee'].isin(X_train['code_insee']).all():
        print("Le fichier communes_features contient des communes non présentes dans X_train")
    ...
    for date in communes_features['annee'].unique():
        for code_insee in communes_features['code_insee'].unique():
            df = communes_features[(communes_features['annee'] == date) & (communes_features['code_insee'] == code_insee)]

            if not df.empty:
                for col in communes_features.columns:
                    if communes_features[col].dtype == 'object':
                        X_train.loc[(X_train['semaine'] // 100 == date) & (X_train['code_insee'] == code_insee), 'com_' + col] = str(df[col].values[0])
                        X_test.loc[(X_test['semaine'] // 100 == date) & (X_test['code_insee'] == code_insee), 'com_' + col] = str(df[col].values[0])
                    else:
                        X_train.loc[(X_train['semaine'] // 100 == date) & (X_train['code_insee'] == code_insee), 'com_' + col] = df[col].values[0]
                        X_test.loc[(X_test['semaine'] // 100 == date) & (X_test['code_insee'] == code_insee), 'com_' + col] = df[col].values[0]
    ...
    variability_features = variability_features.dropna(subset=['pollution_code_insee'])

    if not variability_features['pollution_code_insee'].isin(X_train['code_insee']).all():
        print("Le fichier des variability_features contient des communes non présentes dans X_train")

    variability_features['date'] = variability_features['cal_annee'] * 100 + variability_features['cal_semaine']
    variability_features = variability_features.rename(columns={'pollution_code_insee': 'code_insee'})
    ...
    for col in variability_features.columns:
        if variability_features[col].dtype == 'float64':
            if variability_features[col].apply(lambda x: x.is_integer()).all():
                variability_features[col] = variability_features[col].astype(int)
    ...
    for date in variability_features['date'].unique():
        for code_insee in variability_features['code_insee'].unique():
            df = variability_features[(variability_features['date'] == date) & (variability_features['code_insee'] == code_insee)]

            if not df.empty:
                for col in variability_features.columns:
                    if variability_features[col].dtype == 'object':
                        X_train.loc[(X_train['semaine'] == date) & (X_train['code_insee'] == code_insee), 'var_' + col] = str(df[col].values[0])
                        X_test.loc[(X_test['semaine'] == date) & (X_test['code_insee'] == code_insee), 'var_' + col] = str(df[col].values[0])
                    else:
                        X_train.loc[(X_train['semaine'] == date) & (X_train['code_insee'] == code_insee), 'var_' + col] = df[col].values[0]
                        X_test.loc[(X_test['semaine'] == date) & (X_test['code_insee'] == code_insee), 'var_' + col] = df[col].values[0]
    ...
    print("-> Datasets saved in 'data' folder as X_train.h5, X_test.h5, y_train.h5 and y_test.h5")